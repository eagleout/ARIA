...
-- ============================================
-- BOOTSTRAP: première connexion utilisateur
-- ============================================
-- Permet à un user auth de créer son org + profil sans service_role_key
CREATE OR REPLACE FUNCTION bootstrap_user(p_full_name TEXT DEFAULT 'Admin')
RETURNS JSON AS $$
DECLARE
  v_user_id UUID;
  v_email TEXT;
...
BEGIN
  ...
  IF FOUND THEN
    RETURN json_build_object('organization_id', v_existing_user.organization_id, 'user_id', v_user_id, 'status', 'existing');
  END IF;
  ...
  VALUES (p_full_name || '''s Organization', 'org-' || LEFT(v_user_id::TEXT, 8), 'free')
  RETURNING id INTO v_org_id;

  -- Créer le profil utilisateur (rôle OWNER pour le premier)
  INSERT INTO users (id, organization_id, email, full_name, role)
  VALUES (v_user_id, v_org_id, v_email, p_full_name, 'OWNER');

  RETURN json_build_object('organization_id', v_org_id, 'user_id', v_user_id, 'status', 'created');
END;
