...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Vérifier si le profil existe, sinon bootstrap
    try {
      const checkRes = await fetch('/api/setup')
      const checkData = await checkRes.json()
      if (checkData.status === 'needs_profile') {
        // Auto-bootstrap avec l'email comme nom
        await fetch('/api/setup', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ fullName: email.split('@')[0] }),
        })
      } else if (checkData.status === 'needs_schema') {
        setLoading(false)
        router.push('/setup')
        return
      }
